...
@section('content')
    <section class="grid gap-6 xl:grid-cols-[minmax(0,1.05fr)_minmax(360px,0.95fr)]">
        <div class="space-y-6">
            <article class="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
                <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                    <div>
                        <p class="text-sm font-semibold uppercase tracking-[0.24em] text-slate-400">Order Reference</p>
                ...
                <div class="mt-6 grid gap-4 md:grid-cols-2">
                    ...
                    <div class="rounded-3xl bg-slate-50 p-5">
                        <p class="text-xs font-semibold uppercase tracking-[0.24em] text-slate-400">Shipping</p>
            ...
            <article class="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
                <div class="flex items-center justify-between gap-4">
                    <div>
                        <p class="text-sm font-semibold uppercase tracking-[0.24em] text-slate-400">Order Items</p>
                        <h3 class="mt-2 text-2xl font-black tracking-tight text-slate-950">Snapshot item order</h3>
                    </div>
                </div>
            ...
            <article class="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
                <div class="flex items-center justify-between gap-4">
                    <div>
                        <p class="text-sm font-semibold uppercase tracking-[0.24em] text-slate-400">Inventory Trace</p>
                        <h3 class="mt-2 text-2xl font-black tracking-tight text-slate-950">Log inventory terkait order ini</h3>
                    </div>
                </div>
                ...
                <div class="mt-6 space-y-3">
                        ...
                        <div class="rounded-3xl border border-slate-200 px-4 py-4">
                            <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                                <div>
                                    ...
                                    <p class="mt-1 text-sm text-slate-500">{{ $log->type }} • {{ $log->quantity_changed_label }} • after {{ $log->quantity_after }}</p>
                                </div>
                    ...
                    @empty
                        <div class="rounded-3xl border border-dashed border-slate-200 bg-slate-50 px-5 py-8 text-center text-sm text-slate-500">
                            Belum ada inventory log yang terkait langsung dengan order ini.
                        </div>
                    @endforelse
                </div>
            </article>
        ...
        <div class="space-y-6">
            <article class="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
                <p class="text-sm font-semibold uppercase tracking-[0.24em] text-slate-400">Update Status</p>
                <h3 class="mt-2 text-2xl font-black tracking-tight text-slate-950">Ubah lifecycle order</h3>
                ...
                <form method="POST" action="{{ route('admin.orders.update-status', $order) }}" class="mt-6 space-y-4">
                    ...
                    <div>
                        <label for="order_status" class="text-sm font-semibold text-slate-900">Order Status</label>
            ...
            <article class="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
                <p class="text-sm font-semibold uppercase tracking-[0.24em] text-slate-400">Payment Status</p>
                    ...
                    @foreach ($order->payments as $payment)
                        <div class="mt-4 rounded-3xl bg-slate-50 p-5">
                            ...
                            <div class="mt-4 space-y-2 text-sm text-slate-600">
                                <p><span class="font-semibold text-slate-900">Transaction ID:</span> {{ $payment->transaction_id ?? '-' }}</p>
                                <p><span class="font-semibold text-slate-900">Transaction Status:</span> {{ $payment->transaction_status ?? '-' }}</p>
                                <p><span class="font-semibold text-slate-900">Fraud Status:</span> {{ $payment->fraud_status ?? '-' }}</p>
                                <p><span class="font-semibold text-slate-900">Gross Amount:</span> {{ $payment->gross_amount_label }}</p>
                            </div>
                        </div>
                    @endforeach
                @else
                    <div class="mt-4 rounded-3xl border border-dashed border-slate-200 bg-slate-50 px-5 py-8 text-center text-sm text-slate-500">
                        Belum ada payment record untuk order ini.
                    </div>
                @endif
            </article>

            <article class="rounded-[2rem] border border-slate-200 bg-slate-950 p-6 text-white shadow-xl shadow-slate-900/10">
                <p class="text-sm font-semibold uppercase tracking-[0.24em] text-slate-400">Financial Summary</p>
